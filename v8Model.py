...
import matplotlib
matplotlib.use('Agg')  # Use non-interactive backend to suppress popups
...
DEFAULT_CONFIG = {
    ...
    # Hyperparameters (V8 TUNED)
    "WINDOW_SIZE": 60, 
    "ADX_THRESHOLD": 30,        # V8: Increased from 25 to reduce false bear signals
    "JUMP_THRESHOLD": 3.5, 
    "TARGET_VOL": 0.40,
    
    # V8 NEW: Momentum Override
    "MOMENTUM_LOOKBACK": 50,    # Days for momentum calculation
    "MOMENTUM_THRESHOLD": 0.15, # 15% return triggers momentum override
    
    # V8 NEW: Minimum position during bear trend  
    "BEAR_TREND_MIN_POS": 0.25, # Minimum 25% in bear trend (was 0)
    "BEAR_TREND_MAX_POS": 0.75, # Maximum 75% in bear trend
...
class EnsembleManager:
    ...
    def run_backtest(self, start_date=None, end_date=None, plot_results=True):
        ...
        for t in test_indices:
            ...
            ADX_THRESH = self.config['ADX_THRESHOLD']  # V8: Now 30 instead of 25
            MOMENTUM_THRESH = self.config.get('MOMENTUM_THRESHOLD', 0.15)
            ...
            # V8 NEW: Strong momentum override
            is_strong_momentum = (momentum > MOMENTUM_THRESH)
            
            # PRIORITY LOGIC (V8 UPDATED)
            if is_bearish_jump:
                regime, active_agent_name = "JUMP_BEAR", "crisis"
            elif is_bullish_jump:
                regime, active_agent_name = "JUMP_BULL", "trend"
            elif is_strong_momentum and not is_bearish_jump:
                # V8 NEW: Strong momentum overrides conservative signals
                regime, active_agent_name = "MOMENTUM", "trend"
            ...
            # --- V8 IMPROVED OVERRIDES ---
            
            if regime == 'JUMP_BULL' or regime == 'MOMENTUM':
                raw_action = 1.0
            ...
            # --- V8 IMPROVED DEFENSIVE OVERRIDE ---
            if self.config['LONG_ONLY']:
                # 1. Bearish Jump (Instant Crash) -> Stay Safe
                if regime == 'JUMP_BEAR':
                    scaled_action = 0.0
                
                # 2. V8 IMPROVED: Bear Trend - Maintain minimum position
                elif regime == 'BEAR_TREND':
                    min_pos = self.config.get('BEAR_TREND_MIN_POS', 0.25)
                    max_pos = self.config.get('BEAR_TREND_MAX_POS', 0.75)
                    base_action = raw_action * self.config['ACTION_SCALER'] * vol_scaler
                    # Keep minimum exposure, cap at max
                    scaled_action = np.clip(base_action, min_pos, max_pos)
                
                # 3. V8 NEW: Momentum regime - Full exposure
                elif regime == 'MOMENTUM':
                    scaled_action = 1.0
                
                # 4. Crisis -> Full Control with minimum floor
                elif regime == 'CRISIS':
                    base_action = raw_action * self.config['ACTION_SCALER'] * vol_scaler
                    # Keep at least 10% exposure even in crisis to avoid missing recoveries
                    scaled_action = max(0.1, base_action)
    ...
    def plot_dashboard(self, res):
        ...
        ax0 = axes[0]
        ax0.plot(res.index, res['Portfolio'], label='Ensemble AI (V8)', color='black', linewidth=2)
        ax0.plot(res.index, res['Bench_Equity'], label='Buy & Hold', color='gray', linestyle='--', alpha=0.6)
        ax0.set_title(f"V8 Equity Curve: {self.config['TARGET_ASSET']}", fontweight='bold')
        ...
        if self.config.get('SAVE_PLOTS', False):
            plot_path = os.path.join(
                ...
                f"v8_backtest_{self.config['TARGET_ASSET']}.png"
            )
...
class HyperparameterOptimizer:
    ...
    def optimize(self, train_start, train_end):
        ...
        inference_grid = {
            "ADX_THRESHOLD": [20, 25, 30, 35, 40],  # V8: Higher default 
...
def run_system(tickers, target, start, end, optimize=True):
    ...
    # Safety Defaults for Single Stock (V8 IMPROVED)
    if target != "SPY":
        ...
        # V8: Less conservative bear settings
    ...
    print("\n>>> INITIALIZING TRINITY V8 (IMPROVED) <<<")
