#!/usr/bin/env python3
"""
Multi-Ticker Comprehensive Test for V7 Trading Model
Tests the model across multiple tech tickers to evaluate robustness
Added retry logic to handle Yahoo Finance API issues
"""
...
# Tech tickers to test - subset for faster testing
TECH_TICKERS = [
    ("AAPL", "Apple"),
    ("MSFT", "Microsoft"),
    ("GOOG", "Google"),
...
def run_single_ticker_test(ticker_symbol, ticker_name, optimize=False, verbose=True, max_retries=3):
    """Run model on a single ticker and return performance metrics"""
    
    print(f"\n{'='*80}")
    print(f"TESTING: {ticker_name} ({ticker_symbol})")
    print(f"{'='*80}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            config['TRAINING_STEPS'] = 10000  # Reduced for faster testing
...
def run_comprehensive_test(optimize=False):
    """Run tests across all tech tickers and summarize results"""
    
    print("\n" + "="*80)
    print("V7 MODEL COMPREHENSIVE MULTI-TICKER EVALUATION")
...
if __name__ == "__main__":
    ...
    # Set optimize=True for full hyperparameter optimization (slower)
