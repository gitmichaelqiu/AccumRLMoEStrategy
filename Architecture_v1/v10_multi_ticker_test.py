#!/usr/bin/env python3
"""
Multi-Ticker Test for V10 Aggressive Model
Tests momentum override + conviction boost + reduced defensive drag
"""
...
def run_single_ticker_test(ticker_symbol, ticker_name, verbose=True, max_retries=3):
    print(f"\n{'='*80}")
    print(f"V10 TESTING: {ticker_name} ({ticker_symbol})")
    print(f"{'='*80}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            # V10 specific
            config['USE_MOMENTUM_OVERRIDE'] = True
            config['MOMENTUM_OVERRIDE_THRESHOLD'] = 0.15
            config['CONVICTION_THRESHOLD'] = 0.6
            config['CONVICTION_BOOST'] = 1.3
            config['VIX_DEFENSIVE_TRIGGER'] = 30
...
def run_comprehensive_test():
    print("\n" + "="*80)
    print("V10 AGGRESSIVE MODEL EVALUATION")
    print(f"Test Period: {TEST_START} to {TEST_END}")
    print("Key V10 Features:")
    print("  - Momentum Override: Full exposure when 50d momentum > 15%")
    print("  - Conviction Boost: 1.3x when all agents agree")
    print("  - Aggressive Weights: Trend 45%, Recovery 25%")
    print("  - Reduced Defensive: Only active when VIX > 30")
    print("="*80)
    ...
    # Summary
    print("\n" + "="*80)
    print("V10 COMPREHENSIVE SUMMARY")
    print("="*80)
    ...
    if results:
        ...
        print(f"\n{'='*60}")
        print("V10 AGGREGATE STATISTICS")
        print(f"{'='*60}")
