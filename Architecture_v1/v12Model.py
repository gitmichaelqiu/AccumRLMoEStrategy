...
DEFAULT_CONFIG = {
    ...
    "TEST_END": "2025-05-05",  # Can extend to future if needed, will clip to avail data.
    ...
    # V11: Active Risk Management (Disabled for V12 to test soft-switching)
    "RISK_MANAGEMENT": False,
    "RISK_DRAWDOWN_TIER1": -0.10,  # Sell Half
    "RISK_DRAWDOWN_TIER2": -0.20,  # Cash Out
    
    # Output settings
    "SAVE_PLOTS": True,
    "PLOTS_DIR": "./plots",
    
    # V12: HMM/GMM Settings
    "N_REGIMES": 4,
    "REGIME_FEATURES": ["returns", "realized_vol_20d", "vix_norm", "momentum_50d"]
}

# --- REGIME DETECTOR (GMM/HMM) ---
class RegimeDetector:
    def __init__(self, n_components=4, seeds=[42]):
        self.n_components = n_components
        self.model = GaussianMixture(n_components=n_components, covariance_type="full", random_state=seeds[0])
        self.scaler = StandardScaler()
        self.regime_map = {} # Maps Cluster ID -> 'Growth', 'Crisis', etc.
    ...
    def fit(self, df):
        ...
        # Low Vol + Pos Return -> Growth
        # Low Vol + Low Return -> Stagnation
        # High Vol + Mixed Return -> Transition
        
        # We need to map clusters 0-3 to these labels
        # Let's simple-sort by Volatility first
        # 0: Lowest Vol, ... 3: Highest Vol
        
        # But we need multidimensional sorting. 
        # Let's map dynamically based on unscaled means if possible, or interpret scaled means.
        # Actually simplest is to score each cluster:
        # Score = Trend * (1 - Vol) ... no, let's use the explicit definitions
        
        # Get cluster properties (approximate)
        ...
        sorted_by_vol = stats.sort_values(vol_col)
        
        # Highest Vol -> Crisis
        crisis_id = sorted_by_vol.index[-1]
        self.regime_map[crisis_id] = 'Crisis'
        
        # Lowest Vol -> Check returns and momentum
        remaining = sorted_by_vol.index[:-1]
        
        # Finding Growth (Highest Momentum + Positive Return)
        # Using momentum is more stable than daily returns for trend identification
        growth_id = stats.loc[remaining].sort_values(mom_col).index[-1]
        self.regime_map[growth_id] = 'Growth'
        
        # Remaining two: Transition vs Stagnation
        # Transition has higher vol than Stagnation
        remaining = [c for c in remaining if c != growth_id]
        if len(remaining) == 2:
            # Sort by vol
            if stats.loc[remaining[0], vol_col] > stats.loc[remaining[1], vol_col]:
                self.regime_map[remaining[0]] = 'Transition'
                self.regime_map[remaining[1]] = 'Stagnation'
            else:
                self.regime_map[remaining[1]] = 'Transition'
                self.regime_map[remaining[0]] = 'Stagnation'
        elif len(remaining) == 1:
             self.regime_map[remaining[0]] = 'Stagnation' # Default fallback
             
        print("Regime Mapping (Cluster -> Name):", self.regime_map)
        print("Regime Mapping (Cluster -> Name):", self.regime_map)
        print(stats[[ret_col, vol_col, mom_col]])

    def predict_proba(self, df):
        X = self.scaler.transform(df)
        probs = self.model.predict_proba(X)
        return probs # shape (n_samples, n_components)
    
    def get_regime_weights(self, probs):
        # Map probabilities to Agent weights
        # Mapping:
        # Growth -> Trend Agent
        # Stagnation -> Mean Rev Agent
        # Crisis -> Defensive Agent (Short)
        # Transition -> Recovery Agent (or Cash/Hedge)
        
        # Output weights: {'trend': w1, 'mean_rev': w2, 'defensive': w3, 'recovery': w4}
        weights = {'trend': 0.0, 'mean_rev': 0.0, 'defensive': 0.0, 'recovery': 0.0}
        
        for i in range(self.n_components):
            regime = self.regime_map.get(i, 'Stagnation')
            prob = probs[i]
            ...
            if regime == 'Growth':
                ...
                # Transition logic: Mixed exposure. 
                # Framework says: "Breakout / Long Vol". V11 Recovery agent is closest to Breakout?
                # Or maybe split between Trend and Defensive?
                # Let's use Recovery Agent as the proxy for "Transition" expert 
                # (since it's trained on recovery/vol periods)
                weights['recovery'] += prob
...
class DataProcessor:
    ...
    def download(self, start, end):
        try:
            ...
            # Create a session with chrome impersonation to bypass rate limits/SSL issues
            session = requests.Session(impersonate="chrome")
            
            data = yf.download(self.tickers, start=start, end=end, progress=False, session=session)
            ...
            ohlc = yf.download(self.config['TARGET_ASSET'], start=start, end=end, progress=False, session=session)
            return data, ohlc
    ...
    def add_features(self, df, ohlc):
        ...
        # V11 Feature: 252-day Drawdown for Risk Management
...
class EnsembleManager:
    def __init__(self, config):
        ...
        self.regime_detector = RegimeDetector(n_components=config['N_REGIMES'], seeds=config['SEEDS'])
        
    def train_specialists(self, verbose=True):
        if verbose: print("\n=== 1. TRAINING V12 AGENTS & REGIME DETECTOR ===")
        
        # 1. Train Regime Detector
        all_data = self.dp.get_data(self.config['TRAIN_START'], self.config['TRAIN_END'])
        # Extract features for GMM
        features = all_data[self.config['REGIME_FEATURES']].fillna(0)
        self.regime_detector.fit(features)
        
        # 2. Train Agents
        trend_data = all_data # Use all data for now, let regime weighting handle specialization? 
        # Or should we split data using GMM? 
        # For V12 Step 1, let's keep V11 training logic (specialized periods) 
        # to ensure agents have distinct behaviors.
    ...
    def calculate_weights(self, indicators):
        # NOTE: In V12, this is effectively replaced by `get_regime_weights` inside the loop
        # using the HMM/GMM probabilities.
        pass
    ...
    def run_backtest(self, start_date=None, end_date=None, plot_results=True):
        ...
        if plot_results: print(f"=== 2. RUNNING V12 REGIME-AWARE BACKTEST ({s_date} to {e_date}) ===")
        ...
        for t in test_indices:
            ...
            # V11: RISK MANAGER OVERRIDE
            # Checks for deep drawdown condition to force cash or reduce size
            risk_override_active = False
            
            if self.config.get('RISK_MANAGEMENT', False) and idx_dd252 != -1:
                dd_252 = data_vals[t-1, idx_dd252]
                sma_dist = data_vals[t-1, idx_sma]
                
                # TIER 2: Deep Drawdown -> Force Cash (User request: "turn stock into money")
                if dd_252 < self.config['RISK_DRAWDOWN_TIER2']:
                    final_action = 0.0 # Will be ignored by hard cap but good for logic
                    override_flag = "RISK_MAX"
                    risk_override_active = True
                    risk_trigger_count += 1
                
                # TIER 1: Warning Zone -> Sell Half (User request: "sell half")
                # Only if price is also below SMA200 (confirming downtrend)
                elif dd_252 < self.config['RISK_DRAWDOWN_TIER1'] and sma_dist < 0:
                    override_flag = "RISK_MID"
                    risk_override_active = True 
                    risk_trigger_count += 1
            
            if risk_override_active and override_flag == "RISK_MAX":
                final_action = 0.0 # Placeholder, hard cap handles it below
            else:
                # Normal Logic (with potential Risk Cap)
                ...
                if is_bearish_jump:
                    ...
                    raw_action = self.agents['defensive'].predict(obs_raw, deterministic=True)
                    final_action = raw_action
                    override_flag = "JUMP-"
                elif is_bullish_jump:
                    # Bullish Jump -> Force Trend Agent
                    raw_action = self.agents['trend'].predict(obs_raw, deterministic=True)
                    final_action = raw_action
                    override_flag = "JUMP+"
                if self.config['USE_MOMENTUM_OVERRIDE'] and momentum > self.config['MOMENTUM_OVERRIDE_THRESHOLD']:
                    ...
                    # V12: REGIME-BASED SOFT VOTING
                    # 1. Get current features for GMM
                    # We need a dataframe row for predict_proba
                    
                    # Construct feature vector matching training columns
                    # ["returns", "realized_vol_20d", "vix_norm"]
                    
                    # NOTE: returns at t-1 is valid (known at open of t? No, close of t-1).
                    # 'returns' in dp.add_features is t / t-1 - 1. So t-1 index has returns of t-1. Correct.
                    
                    curr_feats = {
                        'returns': data_vals[t-1, idx_ret],
                        'realized_vol_20d': data_vals[t-1, idx_raw_vol],
                        'vix_norm': data_vals[t-1, idx_vix],
                        'momentum_50d': data_vals[t-1, idx_momentum]
                    }
                    curr_df = pd.DataFrame([curr_feats])
                    
                    # 2. Get Probabilities
                    probs = self.regime_detector.predict_proba(curr_df)[0]
                    
                    # 3. Get Weights
                    weights = self.regime_detector.get_regime_weights(probs)
                    ...
                    if not risk_override_active: 
                        # Show dominant regime as flag
                        max_regime_idx = np.argmax(probs)
                        override_flag = self.regime_detector.regime_map.get(max_regime_idx, "?")[:4]
            ...
            # V11: APPLY RISK LIMITS (Hard Cap on Exposure)
            exposure_cap = self.config['MAX_LEVERAGE']
            if risk_override_active:
                if override_flag == "RISK_MAX": 
                    exposure_cap = 0.0
                elif override_flag == "RISK_MID": 
                    exposure_cap = 0.5
        ...
        if plot_results:
            ...
            print(f"\n=== V12 FINAL REPORT: {total_ret:.2%} (Bench: {bench_ret:.2%}) ===")
            print(f"Alpha: {(total_ret - bench_ret):.2%}")
            print(f"Momentum Overrides: {momentum_override_count}")
            print(f"Risk Triggers: {risk_trigger_count}")
